...
function wrapConversationHistory(history: Array<{ role?: string; text?: string }> | undefined) {
...
const diagnosticQuestionsAiOutputSchema = z.object({
  questions: z.array(questionAiSchema.extend({ moduleId: z.string().optional() })).min(1).max(52),
});
...
function finalizeQuestionsOutput(
  ...
  const minAcceptable = expectedCount <= 3
    ? expectedCount
    : Math.max(1, Math.ceil(expectedCount * 0.85));

  if (finalized.length < minAcceptable) {
...
function aiSchemaForAction(action: string) {
  ...
  if (action === "generateDiagnosticQuestions") return diagnosticQuestionsAiOutputSchema;
  if (
    action === "generatePracticeQuestions"
    || action === "generateInterleavedQuestions"
    || action === "generateJourneyChallenge"
...
async function generateDiagnosticQuestionsBatch(
  ...
  for (const mod of modules) {
    ...
    if (finalized.length < Math.max(1, questionsPerModule - 1)) {
      throw new Error(
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      console.error("[aiStudy/handler]", err, {
        message,
